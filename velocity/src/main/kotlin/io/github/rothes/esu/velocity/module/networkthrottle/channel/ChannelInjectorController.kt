...
object ChannelInjectorController {
    ...
    fun registerInjector(channelInjector: ChannelInjector): Boolean {
        return allInjectors.add(channelInjector).also {
            if (it) forEachInjectAcceptors { injector, channel, player ->
                injector.doInject(channel, player)
            }
        }
    }
    fun unregisterInjector(channelInjector: ChannelInjector): Boolean {
        return allInjectors.remove(channelInjector).also {
            if (it) forEachInjectAcceptors { injector, channel, player ->
    ...
    fun onPostLogin(e: PostLoginEvent) {
        ...
        // Re-inject, velocity add compression-encoder at this period.
        for (injector in allInjectors) {
            if (injector.reinjectOnConfiguration()) {
                injector.doUninject(channel, player)
                injector.doInject(channel, player)
            }
        }
    ...
    private fun ChannelInjector.doInject(channel: Channel, player: Player?) {
        try {
    ...
    private fun ChannelInjector.doUninject(channel: Channel, player: Player?) {
        try {
    ...
    private object EsuChannelInjector: ChannelInjector {
        ...
        override fun reinjectOnConfiguration(): Boolean {
            // If we don't do this, we may not get packet type property after velocity compression-encoder added.
            return true
        }

        override fun inject(channel: Channel, player: Player?) {
        ...
        override fun uninject(channel: Channel, player: Player?) {
    ...
    private class EsuChannelInitializer(val wrapped: ChannelInitializer<Channel>): ChannelInitializer<Channel>() {
